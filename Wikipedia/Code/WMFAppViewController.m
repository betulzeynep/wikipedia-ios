...
- (void)viewDidLoad {
    ...
    [self registerForTraitChanges:@[UITraitUserInterfaceStyle.class]
                      withHandler:^(__kindof WMFAppViewController *_Nonnull viewController, UITraitCollection *_Nonnull previousTraitCollection) {
...
- (void)loadMainUI {
    ...
    if ([NSUserDefaults standardUserDefaults].defaultTabType == WMFAppDefaultTabTypeSettings) {
...
- (WMFComponentNavigationController *)rootNavigationControllerWithRootViewController:(UIViewController *)rootViewController {
    WMFComponentNavigationController *navigationController = [[WMFComponentNavigationController alloc] initWithRootViewController:rootViewController modalPresentationStyle:UIModalPresentationOverFullScreen customBarBackgroundColor:nil];
    ...
    if (!error) {
        ...
        if ([[NSDate date] timeIntervalSinceDate:self.syncStartDate] >= 5) {
            ...
            if (syncedReadingListsCount > 0 && syncedReadingListEntriesCount > 0) {
                ...
                [[WMFToastManager sharedInstance] showToast:alertTitle sticky:YES dismissPreviousToasts:YES tapCallBack:nil];
            }
        }
    }
}
...
- (void)descriptionEditWasPublished:(NSNotification *)note {
    ...
    [[WMFToastManager sharedInstance] showRichToast:@"Your edit was successfully published" subtitle:nil buttonTitle:nil image:[UIImage imageNamed:@"published-pencil"] duration:nil dismissPreviousToasts:YES tapCallBack:nil buttonCallBack:nil completion:nil];
}
...
- (void)performDatabaseHousekeepingWithCompletion:(void (^_Nonnull)(NSError *_Nullable))completion {
    ...
    [SharedContainerCacheHousekeeping deleteStaleCachedItemsIn:SharedContainerCacheCommonNames.didYouKnowCache cleanupLevel:WMFCleanupLevelLow];
...
- (BOOL)processUserActivity:(NSUserActivity *)activity animated:(BOOL)animated completion:(dispatch_block_t)done {
    ...
    switch (type) {
        ...
        case WMFUserActivityTypePlaces: {
            ...
            NSNumber *latitude = [activity wmf_placesLatitude];
            NSNumber *longitude = [activity wmf_placesLongitude];
...
- (nonnull WMFComponentNavigationController *)settingsNavigationController {
    if (!_settingsNavigationController) {
        WMFComponentNavigationController *navController = [[WMFComponentNavigationController alloc] initWithRootViewController:self.settingsViewController modalPresentationStyle:UIModalPresentationOverFullScreen customBarBackgroundColor:nil];
